# ELEVATED: add Sunshine's VDD HDR workaround dd_wa_hdr_toggle_delay = 500 (one change).
$ErrorActionPreference='Continue'
$b='C:\jacks\AI\sunshine-virtual-monitor'; $log="$b\fix10-result.txt"
function L($m){ "{0}  {1}" -f (Get-Date -Format HH:mm:ss),$m | Tee-Object -FilePath $log -Append }
"=== FIX10 START $(Get-Date -Format o) ===" | Set-Content $log
$conf='C:\Program Files\Sunshine\config\sunshine.conf'
Copy-Item $conf "$conf.bak-fix10-$(Get-Date -Format yyyyMMdd-HHmmss)" -Force
$lines = Get-Content $conf | ForEach-Object { if($_ -match '^\s*dd_wa_hdr_toggle_delay\s*=') { 'dd_wa_hdr_toggle_delay = 500' } else { $_ } }
if (-not ($lines -match '^\s*dd_wa_hdr_toggle_delay\s*=')) { $lines += 'dd_wa_hdr_toggle_delay = 500' }
...
Start-Sleep 3
Get-Content 'C:\Program Files\Sunshine\config\sunshine.log' | Select-String "config: 'dd_wa_hdr_toggle_delay'|config: 'dd_hdr_option'" | Select -Last 2 | %{ L ("  "+$_.Line.Substring($_.Line.IndexOf('config:'))) }
...
L "RESULT=$(if($up -and $crash -eq 0){'READY - reconnect and check the tint'}else{'PROBLEM'})"
"=== FIX10 END $(Get-Date -Format o) ===" | Tee-Object -FilePath $log -Append
